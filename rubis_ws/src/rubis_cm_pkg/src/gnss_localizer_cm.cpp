...
Nmea2TFPoseNode::Nmea2TFPoseNode()
  ...
  //, pos_x_init_flag_(false)
  //, pos_y_init_flag_(false)
...
void Nmea2TFPoseNode::initForROS()
{
  ...
  // CarMaker subscribe
  sub1_ = nh_.subscribe("/gps_out", 100, &Nmea2TFPoseNode::callbackFromNmeaSentence, this);
  sub2_ = nh_.subscribe("imu_out", 100, &Nmea2TFPoseNode::callbackFromIMU, this);
...
{ 
  ...
  // if(abs(cur_x) >= 30000 && pos_x_init_flag_ == false){
  //   return;
  // }
  // else if(abs(cur_x) >= 30000){
  //   cur_x = prev_x_;
  // }
  
  // if(abs(cur_y) >= 30000 && pos_y_init_flag_ == false){
  //   return;
  // }
  // else if(abs(cur_y) >= 30000){
  //   cur_y = prev_y_;
  // }
 
  cur_pose_.pose.position.x = cur_x;
  cur_pose_.pose.position.y = cur_y;
  cur_pose_.pose.position.z = cur_z;
  */

  //std::cout<<"Before TransformPose"<<std::endl;
  ...
  // if(abs(cur_x) < 30000){
  //   prev_x_ = cur_x;
  //   if(pos_x_init_flag_==false) pos_x_init_flag_=true;
  // }
  // if(abs(cur_y) < 30000){
  //   prev_y_ = cur_y;
  //   if(pos_y_init_flag_==false) pos_y_init_flag_=true;
  // }
...
void Nmea2TFPoseNode::convert(std::vector<std::string> nmea, ros::Time current_stamp)
{
  try
  {
    ...
    else if (nmea.at(0).compare(3, 3, "GGA") == 0)
    {
      ...
      if (nmea.at(5) == "W")
        lon = -lon;
      std::cout<<"latitude : "<<lat <<" longitude : "<<lon<<" altitude : "<<h<<std::endl;
...
void Nmea2TFPoseNode::callbackFromNmeaSentence(const hellocm_msgs::GPS_Out &msg)
{
//   std::cout<<msg->sentence<<std::endl;
  //convert(split(msg->sentence), msg->header.stamp);
  geo_.set_llh_nmea_degrees(msg.latitude, msg.longitude, msg.altitude);

  double timeout = 10.0;
  //std::cout<<"callback!"<<std::endl;
  
  publishPoseStamped();
  //std::cout << "After_PoseStamped" <<std::endl;
...
//   std::cout<<msg->sentence<<std::endl;
  ...
  //std::cout << "cur_pose_data.x : " << cur_pose_data_.x << " cur_pose_data.y : " << cur_pose_data_.y << " cur_pose_data.z : " << cur_pose_data_.z <<std::endl;
